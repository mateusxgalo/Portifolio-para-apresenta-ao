...
function typeWriter() {
    ...
    if (!deleting) {
        ...
        cIdx++;
        if (cIdx === phrase.length) { deleting = true;
            setTimeout(typeWriter, 1800); return; }
        setTimeout(typeWriter, 80);
    } else {
        el.textContent = phrase.substring(0, cIdx - 1);
        cIdx--;
        if (cIdx === 0) { deleting = false;
...
function toggleChallenge(i, el) {
    if (done.has(i)) { done.delete(i);
        ...
        el.querySelector('.challenge-check').textContent = ''; } else { done.add(i);
        el.classList.add('done');
        el.querySelector('.challenge-check').textContent = '✓'; }
